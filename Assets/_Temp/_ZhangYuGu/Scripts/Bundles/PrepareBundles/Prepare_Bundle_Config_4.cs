 ...
 */ 
public class Prepare_Bundle_Config_4 : MonoBehaviour {
	...
	private static Prepare_Bundle_Config_4 m_instance = null;

	#region Mono

	public static Prepare_Bundle_Config_4 Instance(){
		if( m_instance == null ){
			Debug.LogError( "Error, Prepare_Bundle_Config_4.m_instance = null." );
		}
...
//					Debug.Log( "Waiting For Response." );
			...
			// check
			{
				...
				if( IsToUpdateBundle() ){
					if( Bundle_Loader.m_bundle_list_info.m_big_version != VersionTool_4.GetBigVersion() ){
...
//				Debug.Log( "Save Version: " + Bundle_Loader.m_bundle_list_info.m_small_version );
				...
				PlayerPrefs.SetString( ConstInGame.CONST_PLAYER_PREFS_KEY_CACHED_BUNDLE_BIG_VRESION, VersionTool_4.GetBigVersion() );
	...
	private void UpdateServerBundleInfo(){
		...
		
		{
			...
			
			{
				...
				t_request_params.Add( "bigVersion", VersionTool_4.GetBigVersion() );
	...
	public void UpdateBundleCallback( ref WWW p_www, string p_path, Object p_object ){
		{
			...
			LoadingHelper.ItemLoaded( m_loading_sections, CONST_UPDATE_BUNDLES );
...
//		Debug.Log( "CleanBundleUpdateList()" );
	...
	private void ExecExtractBundles(){
...

		{
			...
			LoadingHelper.InitSectionInfo( m_loading_sections, CONST_EXTRACT_ALL_BUNDLES, 100, m_bundles_to_extract_count );
		}
	...
	public void BundleExtractedCallback( ref WWW p_www, string p_path, UnityEngine.Object p_object ){
		{
			...
			LoadingHelper.ItemLoaded( m_loading_sections, CONST_EXTRACT_ALL_BUNDLES );
...
//			SetLoadingTitle( "Update Assets.." );
	...
	private void LoadCachedBundleVersions(){
		...
		
		{
			...
			string t_cur_big_version = VersionTool_4.GetBigVersion();
		...
		m_config_package_small_version = VersionTool_4.GetSmallVersion();
	}
	...
	public void LoadingDataDone(){
		{
			...
			LoadingHelper.ItemLoaded( m_loading_sections, CONST_LOADING_PRELOAD_RESOURCES );
...
//		}
	...
	private static void InitLoadingSections(){
		...
		case BundleState.ToUpdate:
			LoadingHelper.InitSectionInfo( m_loading_sections, CONST_UPDATE_BUNDLES, 1, GetUpdateBundleCount() );
			break;
		}

		{
			LoadingHelper.InitSectionInfo( m_loading_sections, CONST_LOADING_BUNDLE_CONFIG, 1, GetLoadingBundleConfigCount() );
			
			LoadingHelper.InitSectionInfo( m_loading_sections, CONST_LOADING_PRELOAD_RESOURCES, 1, PREPARE_DATA_COUNT_FOR_LOADING );
		}
	}
