...
public class CameraTestControllor : MonoBehaviour
{
	public CameraTestCamera gameCamera;
	...
	public void moveKing(Vector3 offset)
	{
		if( Vector3.Distance(offset, Vector3.zero) != 0 
		   && gameCamera != null 
		   && gameCamera.transform.localEulerAngles.y != 0){ //摄像机角度不同时的修正
			...
			camObject.transform.localEulerAngles = new Vector3(0, gameCamera.transform.localEulerAngles.y, 0);
			
			offset = offObject.transform.position;
		}
		
		if(Vector3.Distance(offset, Vector3.zero) != 0){ //转向时的角速度
			...
			angleObject.transform.eulerAngles = transform.eulerAngles;
...
//		if(offset.x != 0) offset += new Vector3(offset.x * 0.1f, 0, 0);
	...
	{
		if (Vector3.Distance (offset, Vector3.zero) < .2f) return;

		transform.forward = offset;

		character.Move(offset * 7f * Time.deltaTime);
	}
