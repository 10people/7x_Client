...
#elif !UNITY_3_5
				if (!mKeyboard.wasCanceled && mKeyboard.done)
#endif
					Submit();
				mKeyboard = null;
				isSelected = false;
...
//		Debug.Log ( "Cleanup( " + " )" );
	...
	{
//		Debug.Log ( "Validate( " + text + ", " + pos + ", " + ch + " )" );
...
//			if (ch>=0x4e00 && ch<=0x9fa5) return ch;//这个主要是汉字的范围
		...
		else if (validation == Validation.CreateRoleName)
		{
			if (ch >= 'A' && ch <= 'Z') return ch;
			if (ch >= 'a' && ch <= 'z') return ch;
			if (ch >= '0' && ch <= '9') return ch;
		...
		else if (validation == Validation.Alliance_Notice)
		{
			if (ch >= 'A' && ch <= 'Z') return ch;
			if (ch >= 'a' && ch <= 'z') return ch;
			if (ch >= '0' && ch <= '9') return ch;
