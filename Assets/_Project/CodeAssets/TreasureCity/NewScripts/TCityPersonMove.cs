...
public class TCityPersonMove : MonoBehaviour {
	...
	{
		if (Vector3.Distance(targetPosition, transform.position) < 0.6f) return;
	...
	void Update()
	{
		if (m_MoveType == MoveType.NONE) _playerRandomAnimator = true;
		
		if (_isRun && Mathf.Abs (Vector3.Distance (_TargetPos,transform.position)) > 0)
		{
			if (Mathf.Abs (Vector3.Distance (_TargetPos, transform.position)) > 0.2f)
			{
				Move (_TargetPos);
			}
			else
	...
	void Move(Vector3 point)
	{
		...
		{
			transform.localPosition = Vector3.Lerp(transform.position, point, TreasureCityPlayer.m_instance.m_speed * Time.deltaTime * 0.3f);
		}
	}
	...
	void MoveTurnToDestination(Vector3 targetPosition)
	{
		...
		{
			Vector3 sp2 = targetPosition * TreasureCityPlayer.m_instance.m_speed * Time.deltaTime;
