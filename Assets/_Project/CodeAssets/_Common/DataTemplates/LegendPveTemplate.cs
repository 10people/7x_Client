...
public class LegendPveTemplate : XmlLoadManager {

	
	
	// <PveTemp id="100101" bigName="30001" bigDesc="30001" bigId="1" smaName="40101" 
	//smaDesc="40101" smaId="1" chapType="0" useHp="5" monarchLevel="1" frontPoint="0" 
	//openCondition="0" money="100" exp="50" awardId="" firstAwardId="0" npcId="101010" 
	//bossId="0" landId="0" power="100" star1="0" star2="0" star3="0" icon="4" 
	//soundId="100000" sceneId="1" />
	...
	public void Log(){
		Debug.Log( "LegendPveTemplate-  id: " + id +
		          " bigName: " + bigName + 
		          " smaName: " + smaName );
	}
	...
	public static void LoadTemplates( EventDelegate.Callback p_callback = null ){
		UnLoadManager.DownLoad(PathManager.GetUrl(m_LoadPath + "LegendPveTemp.xml"), CurLoad, UtilityTool.GetEventDelegateList( p_callback ), false );
	}
	...
	public static void CurLoad(ref WWW www, string path, Object obj){
		...
		do{
			t_has_items = t_reader.ReadToFollowing( "LegendPveTemp" );
			
			if( !t_has_items ){
				break;
			}
			
			LegendPveTemplate t_template = new LegendPveTemplate();
	...
	public static LegendPveTemplate GetLegendPVETemplate( int p_section, int p_level ){
		for( int i = 0; i < templates.Count; i++ ){
			LegendPveTemplate t_item = templates[ i ];
		...
		Debug.LogError( "LegendPveTemplate not found: " + p_section + ", " + p_level );
	...
	public static List<LegendPveTemplate> GetlegendPveTemplate_By_bigName( int mbigName ){

		List<LegendPveTemplate> legendlist = new List<LegendPveTemplate> ();

		for( int i = 0; i < templates.Count; i++ ){

			LegendPveTemplate t_item = templates[ i ];
			
			if( t_item.bigName == mbigName ){

				legendlist.Add(t_item);
			}
		}
	...
	public static LegendPveTemplate GetlegendPveTemplate_By_id( int p_level_id ){
		for( int i = 0; i < templates.Count; i++ ){
			LegendPveTemplate t_item = templates[ i ];
	...
	public static LegendPveTemplate GetPveTemplate_By_Chapter_Id( int Chapter_Id ){
		for( int i = 0; i < templates.Count; i++ ){
			LegendPveTemplate t_item = templates[ i ];
