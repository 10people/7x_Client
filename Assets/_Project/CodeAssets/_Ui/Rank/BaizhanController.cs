...
namespace Rank
{
    public class BaizhanController : ModuleController, SocketListener
    {
        ...
        public void Refresh(List<BaiZhanInfo> list, int result)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    var controller = temp.GetComponent<BaizhanDetailController>();
                    controller.m_ModuleController = this;
                    controller.m_BaizhanInfo = list[i];
                    controller.SetThis();
                    ...
                    case ProtoIndexes.RANKING_RESP:
                        {
                            ...
                            if (tempResp.rankType != 3) return false;

                            //Set page.
                            CurrentPageIndex = tempResp.pageNo;
                            TotalPageIndex = tempResp.pageCount;

                            Refresh(tempResp.baizhanList, tempResp.result);
