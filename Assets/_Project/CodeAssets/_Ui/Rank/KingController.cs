...
namespace Rank
{
    public class KingController : ModuleController, SocketListener
    {
        ...
        public void Refresh(List<JunZhuInfo> list, int result)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    controller.m_JunZhuInfo = list[i];
                    controller.SetThis();
            ...
            if (IsOneMode)
            {
                var detailControllerList = m_DetailControllerList.Select(item => item as KingDetailController).Where(item => item.m_JunZhuInfo.name == m_OneModeNameStr).ToList();
                    ...
                    case ProtoIndexes.RANKING_RESP:
                        {
                            ...
                            if (tempResp.rankType != 1) return false;

                            //Set page.
                            CurrentPageIndex = tempResp.pageNo;
                            TotalPageIndex = tempResp.pageCount;

                            Refresh(tempResp.junList, tempResp.result);
