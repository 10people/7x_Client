...
namespace Rank
{
    public class ChongLouController : ModuleController, SocketListener
    {
        ...
        public void Refresh(List<ChongLouInfo> list)
        {
            ...
            if (IsOneMode)
            {
                var detailControllerList = m_DetailControllerList.Select(item => item as ChongLouDetailController).Where(item => item.m_ChongLouInfo.name == m_OneModeNameStr).ToList();
                    ...
                    case ProtoIndexes.RANKING_RESP:
                        {
                            ...
                            if (tempResp.rankType != 7) return false;
