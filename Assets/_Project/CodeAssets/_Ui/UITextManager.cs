...
public class UITextManager
{
	...
	private void initText()
	{
		...
		{
			if(m_listUIText[i].m_panelMove.transform.localPosition.y < ((m_listUIText.Count - 1) - i)  * 40)
			{
				m_listUIText[i].m_panelMove.transform.localPosition = new Vector3(0,((m_listUIText.Count - 1) - i) * 40, 0);
			}
	...
	public void Update ()
	{
		for(int i = 0; i < m_listUIText.Count ; i ++)
		{
			...
			if(m_listUIText[i].m_panelMove.transform.localPosition.y >= 240)
			{
	...
	{
		float tempF = 1 - (uiText.m_panelMove.transform.localPosition.y / 240f);
